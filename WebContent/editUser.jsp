...
<div class="panel-body">
	<form action="editUser" method="post" class="form-horizontal">
		<div class="form-group">
			...
			<div class="col-sm-8">
				<input id="login_id" name="login_id" value="${editUser.loginId}" class="form-control" placeholder="半角英数字 6文字以上20文字以下で入力してください" />
			</div>
		</div>
		<div class="form-group">
			<label for="password" class="control-label col-sm-2">パスワード</label>
			<div class="col-sm-8">
				<input id="password" name="password" type="password" class="form-control" placeholder="必須ではありませんが、変更する場合は10文字以下で入力してください" />
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="col-sm-8">
				<input id="name" name="name" value="${editUser.name}" class="form-control" placeholder="10文字以下で入力してください" />
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-8">
				...
				<c:forEach var="obj" items="${branch}">
					<option value="${obj.key}" <c:if test="${editUser.branchId == obj.key}">selected</c:if>>${obj.value}</option>
				</c:forEach>
				</select>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-8">
				...
				<c:forEach var="obj" items="${department}">
					<option value="${obj.key}" <c:if test="${editUser.departmentId == obj.key}">selected</c:if>>${obj.value}</option>
				</c:forEach>
				</select>
			</div>
		</div>
